...
#include "SpikeTrain_LongDouble.h"
#include "iappDist_LongDouble.h"
...
const long double dt = 0.01;						// Time step: ms
const long double maxTimeSimulation = 8000;  		// Maximum simulation time: ms
const bool SAVE_SIMULATION = true;
const int n_precision = 14;

// Simulation parameters
long double pExcNeurons = 1; 										// Percentage of excitatory neurons
...
const long double thA = 0.1730;										// thA = 0.25*(maxAt-minAt); based on Patrick paper. Previous calculated in Matlab
const long double thDA = 0.1490;
bool activePhase;
...
// Still are not used
const long double timeTotrash = 1000; 				// Time discarded (transient behavior) from the beginning of the simulation,
const long double timeForParameters = 20000; 		// Time to calculate parameters as

// Cellular parameters
// p i0=-10. deli=15
// p vna=115  vk=-12  vl=10.6  gnabar=36  gkbar=12  gl=0.1
// p h0=0.8
long double i0=-10.0, deli=15,
		vna=115, vl=10.6,  gnabar=36,  gkbar=12,  gl=0.1,
		h0=0.8;

// p vexc=70
// p vinh = -12
long double vExc = 70, vsyn = 70,
		vInh = -12, vk = -12;
...
// p Vthresh=40 kv=1
const long double taus=10, tauf=1,
		gsyn=3.6/nNeurons,
...
vector<long double> iapp(nNeurons);	 				// for Excitatory neurons
vector<long double> iapp_i(nNeurons); 				// for Inhibitory neurons
long double iappj;

//# Synaptic drive from excitatory cells
long double atotExc, atotExcj;
//# Synaptic drive from inhibitory cells
long double atotInh, atotInhj;
...
typedef boost::array<long double, 4> NeuronState_v_n_a_s;
typedef boost::array<long double, 4> myArrayDouble4;
...
inline long double am (const long double v) {
	return .1*(25-v)/(exp(.1*(25-v))-1);
}

// bm(v)=4.0*exp(-v/18)
inline long double bm (const long double v) {
	return 4.0*exp(-v/18);
}

// minf(v)=am(v)/(am(v)+bm(v))
inline long double minf (const long double v) {
	return am(v)/(am(v)+bm(v));
}

// bn(v)=.125*exp(-v/80)
inline long double bn (const long double v) {
	return .125*exp(-v/80);
}

// an(v)=.01*(10-v)/(exp(.1*(10-v))-1.0)
inline long double an (const long double v) {
	return .01*(10-v)/(exp(.1*(10-v))-1.0);
}

// Synaptic activation
//fsyn(V) = 1./(1+exp((Vthresh-V)/kv))
inline long double fsyn (const long double v) {
...
int main(int argc, char* argv[]) {
	...
	unsigned int n;
	long double t;
	long double actTotalExc, actTotalInh, v_1, s_t;
	long double sTotalExc, sTotalInh;
	...
	if (SAVE_SIMULATION) {
		...
		fileNameStr = "./results/HH_BBT_" +
				...
				to_string(nInhNeurons) + ",vI" +
				to_string((int)vInh) + ",t=" +
				to_string((int)(maxTimeSimulation/1000)) +"s_LongDouble_IappDES,Epis.txt";
		...
		fileNameStr = "./results/HH_BBT_" +
				...
				to_string(nInhNeurons) + ",vI" +
				to_string((int)vInh) + ",t=" +
				to_string((int)(maxTimeSimulation/1000)) +"s_LongDouble_IappDES,Iapp.txt";
		...
		fileNameStr = "./results/HH_BBT_" +
				...
				negPosVInh +
				to_string((int)(maxTimeSimulation/1000)) + "s_LongDouble_IappDES_Spikes.m";
