...
SpikeTrain::SpikeTrain(const unsigned int n) {
	...
	for(unsigned int i = 0; i < n; i++){
		vector<long double> temp; // create an array, don't work directly on buff yet.
...
void SpikeTrain::addSpikeTimeToNeuron(const unsigned int n, const long double t) {
	...
	long double lastTime = mSpikeTrain[n][mSpikeTrain[n].size()-1];
	if (lastTime < t) {
		mSpikeTrain[n].push_back(t);
	} else
...
string SpikeTrain::toString(const unsigned int n, bool labels = false) const {
	...
	vector<long double> vST = mSpikeTrain[n];

	// List of spike times
	for (vector<long double>::iterator st = vST.begin(); st != vST.end(); ++st) {
