...
int main(int argc, char* argv[]) {
    ...
    if (SAVE_SIMULATION) {
        ...
        string fileNameStr = "./results/HH_BBT_" +
                ...
                actualPrecisionType;
        writeToFile(fileNameStr+"_IappDES.txt", ave);
        
        // Saving the Episodes start/end times
        writeToFile( fileNameStr+"_IappDES,Epis.txt", networkOutputs);

        // Saving the applied currents values
        writeIappToFile(fileNameStr+"_IappDES,Iapp.txt", iapp, n_precision);
        ...
        fileNameStr = "./results/HH_BBT_" +
                ...
                to_string((int)(maxTimeSimulation/1000)) + "s_" +
                actualPrecisionType;
        mSpikeTrain->printToMatlabFile(fileNameStr+"_IappDES_Spikes.m", n_precision);
